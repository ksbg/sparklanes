...
class TestSparkSubmit(TestCase):
    ...
    def test_command_line_args(self):
        ...
        requirements_txt = os.path.join(os.path.dirname(sparklanes.__file__), '_submit', 'requirements-submit.txt')
        ...
        valid_args = [
            ...
            ['-y', iris_yml, '-p', pkg_dir, '-r', requirements_txt, '-m', custom_main, '--extra-data', data_dir],
            ['--yaml', iris_yml, '-p', pkg_dir, '-e', data_dir, '-s'] + spark_args,
            ['-y', iris_yml, '--package', pkg_dir, '--requirements', requirements_txt, '--spark-args'] + spark_args
        ]
        ...
        invalid_args = [
            ...
            ['-y', iris_yml, '-p', pkg_dir, '-r', requirements_txt, '-m', 'A'],  # Non-existent custom main
        ...
        for args in valid_args:
            try:
                ...
                self.fail('Command line parsing should not have failed for the following args: `%s`' % str(args))
